...
4. 
5. 某时刻系统资源总数（7.3.5），剩余可用资源为（2,0,2），此时进程PO需要资源为（5,3,2），已分配（1.1.0），进程P1需要资源为（2,1,1），已分配（1.1.0），进程P2需要资源为（6.0.1），已分配（2.0.1），进程P3需要资源（1,1,1），已分配（1,0,1），进程P4需要资源为（3,2,2），已分配（0,1,1），使用银行家算法计算安全序列，其中不发生死锁的是（）。
